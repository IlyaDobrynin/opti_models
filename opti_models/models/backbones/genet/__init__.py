...
class PlainNet(nn.Module):
    def __init__(self, num_classes=None, plainnet_struct=None, no_create=False, **kwargs):
        ...
        if isinstance(block_list[-1], AdaptiveAvgPool):
            self.adaptive_avg_pool = block_list[-1]
            block_list.pop(-1)
        else:
            self.adaptive_avg_pool = AdaptiveAvgPool(out_channels=block_list[-1].out_channels, output_size=1)

        self.block_list = block_list
        if not no_create:
            self.module_list = nn.ModuleList(block_list)  # register

        self.last_channels = self.adaptive_avg_pool.out_channels
        ...
        self.plainnet_struct = str(self) + str(self.adaptive_avg_pool)
